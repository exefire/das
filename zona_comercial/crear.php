<h2>Crear Zona Comercial</h2>
<? 
if (isset($_POST["nuevo_zona_comercial"])) { 
	foreach($_POST as $key => $value) { 
		$_POST[$key] = mysql_real_escape_string($value);
		$_POST[$key] = utf8_decode($value); 
			} 
	$sql = "INSERT INTO `zona_comercial` ( `nombre` ) 
					VALUES( '".$_POST["nombre"]."' );"; 
	mysql_query($sql) or die(mysql_error()); 
	?><div class="msg">Registro de Zona Comercial agregado.</div><?php 
} 
?>
<form action="<?php echo agrega_get(); ?>" method="post">
<input name="nuevo_zona_comercial" type="hidden" value="1" />
