...
<? 
if (isset($_POST["nuevo_clientes"])) { 
	foreach($_POST as $key => $value) { 
		$_POST[$key] = mysql_real_escape_string($value);
		$_POST[$key] = utf8_decode($value); 
			} 
	$sql = "INSERT INTO `clientes` ( `id_zona_comercial` , `nombre` ) 
					VALUES( '".$_POST["id_zona_comercial"]."', '".$_POST["nombre"]."' );"; 
	mysql_query($sql) or die(mysql_error()); 
	?><div class="msg">Registro de Clientes agregado.</div><?php 
} 
?>
<form action="<?php echo agrega_get(); ?>" method="post">
<input name="nuevo_clientes" type="hidden" value="1" />
